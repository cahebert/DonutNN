...
def create_fully_connected_layer(input_, dim, out=1, relu=False,
                                 name='fully_connected'):
    with tf.name_scope(name):
        flat = tf.reshape(input_, [-1, dim])
...
class CNN():
    def __init__(self, w_shapes, b_shapes, name='', learning_rate=1e-5):
    ...
    def build(self, w_shapes, b_shapes, learning_rate):
        ...
        flattened = tf.reshape(result, [-1,7*7*128])
        fcl1_out = tf.layers.dense(inputs=flattened, units=1024, activation=tf.nn.relu)
        fcl2_out = tf.layers.dense(inputs=fcl1_out, units=13, activation=tf.nn.relu)

        self.coef_pred = tf.reshape(fcl2_out,[-1,13,1])
    ...
    def get_loss(self, donut, coef, i_t, max_chunk=100):
        fetches = [self.loss, self.train_summ]
        if donut.shape[0] < max_chunk:
            result = self.sess.run(fetches, feed_dict={self.donut: donut, self.coef_truth: coef})
            ...
            for i in range(nchunks):
                ...
                loss,result = self.sess.run(fetches, feed_dict={self.donut: donut[j:k], self.coef_truth: coef[j:k]})
    ...
    def test(self, donut,max_chunk=100):
        if donut.shape[0] < max_chunk:
            return self.coef_pred.eval(session=self.sess,feed_dict={self.donut: donut})
        else:
            chunk_coefs = np.zeros((donut.shape[0],13,1))
            nchunks = int(np.ceil(float(donut.shape[0])/max_chunk))
            for i in range(nchunks):
                j, k = i * max_chunk, (i+1) * max_chunk
                pred = self.coef_pred.eval(session=self.sess, feed_dict={self.donut: donut[j:k]})
                chunk_coefs[j:k,:,:] = pred #tf.reshape(pred,[-1,64,64])
            return chunk_coefs
